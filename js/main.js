d3.csv("data/lebron_stats.csv").then(function(data) {
    ...
    //dijagram kao default ima poene
    update("PTS");
    //event listener za promijene izbora
...
d3.csv("data/lebron_stats.csv").then(function(data) {
    ...
    //dijagram za default prikazuje sve statove
    updateChart();
...
d3.csv("data/lebron_stats.csv").then(function(data) {
      ...
      const axis = 5;
      //linearna skala za radial distance, koliko daleko od centra se sta plota
      const r = d3.scaleLinear()
        .range([0, Math.min(width / 2, height / 2)])
        .domain([0, 1]);
      //generator radar linija
      const radarLine = d3.lineRadial()
        .curve(d3.curveLinearClosed)
        .radius(d => r(d.value / maxValues[d.axis]))
        .angle((d, i) => i * angleSlice);
      //dodatne linije na radaru za bolju vizaulizaciju
...
d3.csv("data/lebron_stats.csv").then(function(data) {
  ...
  data.forEach(d => {
    ...
    //definiranje skale boja za heat dijagram
    ...
    //oboja cell ovisni o statu u usporedbi s prosjekom lige
    stats.forEach(stat => {
      cells.append("rect")
...
d3.csv("data/lebron_stats.csv").then(function(data) {
    data.forEach(d => {
      ...
      //brojanje pojave svakih od timova u csv
      ...
      const arcs = svg.selectAll(".arc")
          ...
          .enter().append("g")
          .attr("class", "arc");
      //interaktivnost i crtanje odijeljaka
      arcs.append("path")
      .attr("d", arc)
